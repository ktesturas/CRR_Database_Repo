...
# Create a new column with the formatted label (Status + count)
status_summary <- status_summary %>%
  mutate(Status_Label = paste0(Status, " (", n, ")"))

# Merge back to data, so data can use the new Status_Label in the plot
data <- data %>%
  left_join(status_summary %>% select(Status, Status_Label), by = "Status")
...
mycols <- c("#375E97", "#FFBB00","#FB6542") # example colors

# Plot
ggplot(data_sum, aes(x = 2, y = n, fill = Status_Label)) +
  geom_bar(stat = "identity", color = "white") +
  coord_polar(theta = "y", start = 0) +
  scale_fill_manual(values = mycols) +
  xlim(0.5, 2.5) +  # Donut shape
  theme_void() +
...
# Save Plot
ggsave("Output/Entry_Status.png", width = 7, height = 4, dpi = 720)

#-------------------------------------------------------------------------------
# Plot 2
data2 <- read_excel("Data/20250422 CRR Governance Database 2023-2025.xlsx",
                   sheet = "2023 PubDel Entries")
...
status_summary <- combined_data %>%
  count(visual_status)

# Create formatted legend labels: Status + count
status_summary <- status_summary %>%
  mutate(visual_status_label = paste0(visual_status, " (", n, ")"))

# Attach back to combined_data with new labels
combined_data <- combined_data %>%
  left_join(status_summary %>% select(visual_status, visual_status_label), by = "visual_status")

# Define custom colours - directly matched to the full labels
custom_colours <- setNames(
  c("#FFBB00","#7DCE82",  "#B388EB", "#FB6542"),
  status_summary$visual_status_label
)

# Summarize for plotting
combined_data_sum <- combined_data %>%
  count(visual_status_label)

# Plot
ggplot(combined_data_sum, aes(x = 2, y = n, fill = visual_status_label)) +
  geom_bar(stat = "identity", color = "white") +
  coord_polar(theta = "y", start = 0) +
