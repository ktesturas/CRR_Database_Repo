...
make_motivation_panel <- function(show_x = FALSE, legend = "none") {
  base <- ggplot(prop_mot, aes(x = prop, y = Bioregion, fill = Primary_motivation)) +
    ...
    guides(fill = guide_legend(nrow = 2, byrow = TRUE, reverse = TRUE)) +  # <-- 3 rows, reversed
    labs(x = NULL, y = NULL) +
    theme_minimal(base_size = 14) +
    theme(
      panel.grid.major.y = element_blank(),
...
stacked_like_A_with_mot_legend_only <- cowplot::plot_grid(
  ...
  legend_mot,
  ncol = 1,
  rel_heights = c(1, 0.12)  # adjust legend space as needed
)
stacked_like_A_with_mot_legend_only
# =========================
# Save outputs
# =========================
# ggsave("fig1A_stacked_with_motivation.png",    stacked_no_x_no_legend,                 width = 7, height = 12, dpi = 300)
